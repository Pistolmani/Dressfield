...
import { Package, ShoppingBag, Scissors, ArrowRight, TrendingUp, DollarSign, Clock, AlertCircle } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  const isLoading = loadingOrders || loadingCustom;
  const isError = errorOrders || errorCustom;
  ...
  const revenueToday = orders
    .filter((o) => PAID_STATUSES.includes(o.status) && isToday(o.createdAt))
    .reduce((acc, order) => acc + order.totalAmount, 0);
  
  // Custom orders status 0 is "Pending" (მოლოდინში)
  const pendingCustomOrders = customOrders.filter((co) => co.status === 0);
  const pendingCustomCount = pendingCustomOrders.length;
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-10">
      ...
      {isError && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-red-700 mt-1">მონაცემების ჩატვირთვა ვერ მოხერხდა. სცადეთ მოგვიანებით.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-8">
                ...
                <div className="divide-y divide-border">
                  {recentOrders.map((order) => (
                    <div key={order.id} className="flex items-center justify-between p-4 hover:bg-black/[0.02] transition-colors">
                      <div>
                        <Link href={`/admin/orders/detail?id=${order.id}`} className="font-medium hover:underline text-foreground">
                          #{order.id} {order.contactName}
                        </Link>
                        <p className="text-xs text-muted-foreground mt-0.5">{formatDate(order.createdAt)} · {formatPrice(order.totalAmount)}</p>
                      </div>
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${OrderStatusColors[order.status]}`}>
          ...
          {/* Pending Custom Orders Priority Panel */}
          <section>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold font-ui">ყურადღება (Custom)</h2>
              <Link href="/admin/custom-orders" className="text-sm text-accent hover:underline flex items-center gap-1">
              ...
              {isLoading ? (
                ...
                <div className="divide-y divide-orange-200">
                  {recentPendingCustom.map((co) => (
                    <div key={co.id} className="flex items-center justify-between p-4 hover:bg-orange-50/50 transition-colors">
                      <div>
                        <Link href={`/admin/custom-orders/detail?id=${co.id}`} className="font-medium hover:underline text-foreground">
