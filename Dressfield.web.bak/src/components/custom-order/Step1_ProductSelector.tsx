...
export function Step1ProductSelector({ selected, onSelect }: Step1ProductSelectorProps) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-semibold text-gray-600">აირჩიე პროდუქტის ტიპი</h2>
        <p className="mt-1 text-base text-gray-400">რომელ ტანსაცმელზე გინდა ნაქარგი?</p>
      </div>

      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        {PRODUCT_TYPES.map((product) => {
          const isSelected = selected === product.id;
          const displayLabel = getProductDisplayLabel(product.id, product.label);
          ...
          return (
            <button
              ...
              className={cn(
                "group relative flex cursor-pointer select-none touch-manipulation flex-col items-center gap-3 rounded-2xl border bg-white p-4 transition-[border-color,background-color,box-shadow,transform] duration-100 ease-out hover:shadow-md active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent",
                isSelected
