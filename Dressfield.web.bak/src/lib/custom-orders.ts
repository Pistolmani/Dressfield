...
export async function submitCustomOrder(payload: CreateCustomOrderRequest): Promise<CustomOrderDetailDto> {
  const { data } = await api.post<CustomOrderDetailDto>("/api/custom-orders", payload);
  return data;
}

export async function getAdminCustomOrders(status?: CustomOrderStatus): Promise<CustomOrderSummaryDto[]> {
...
export async function getAdminCustomOrderById(id: number): Promise<CustomOrderDetailDto> {
